...
def tf_idf(T_freq, D_freq, N):
    return T_freq * np.log(N/(1+D_freq))
...
def get_doc_to_norm(index, doc_freq, num_docs):
    ...
    for term in index.keys():
        for doc_id, doc_tf in index[term]:
            doc_norm[doc_id] += tf_idf(doc_tf, doc_freq[term], num_docs) **2
...
def run_query(query_token_counts, index, doc_freq, doc_norm, num_docs):
    ...
    for query_term, query_tf in query_token_counts:
        if query_term not in index:
            continue
        query_norm += tf_idf(query_tf, doc_freq[query_term], num_docs)**2
    ...
    for query_term, query_tf in query_token_counts:
        ...
        for doc_id, doc_tf in index[query_term]:
            doc_to_score[doc_id] += query_tf * doc_tf * (tf_idf(1, doc_freq[query_term], num_docs)**2) / (doc_norm[doc_id] * query_norm)
